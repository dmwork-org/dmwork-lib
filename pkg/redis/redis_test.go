...
// TestMSetMultipleKeyValues tests that MSet correctly sets multiple key-value pairs.
// Before the fix, the []string slice was passed as a single interface{} argument
// instead of being expanded, causing Redis MSet to fail or produce unexpected results.
...
// TestMSetSingleKeyValue tests that MSet works with a single key-value pair.
func TestMSetSingleKeyValue(t *testing.T) {
	addr := os.Getenv("REDIS_ADDR")
	if addr == "" {
		t.Skip("REDIS_ADDR not set, skipping integration test")
	}
	...
	testKey := "test:mset:single"
	defer conn.Del(testKey)

	err := conn.MSet(testKey, "singleValue")
	if err != nil {
		t.Fatalf("MSet failed: %v", err)
	}

	val, err := conn.GetString(testKey)
	if err != nil {
		t.Fatalf("GetString failed: %v", err)
	}
	if val != "singleValue" {
		t.Errorf("expected 'singleValue', got '%s'", val)
	}
}
